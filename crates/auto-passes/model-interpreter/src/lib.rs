//! Distilled-model interpreter — auto-model's inference compiled for
//! artifacts. Same frozen ABI + `init` extension as the DSL interpreter:
//! `init` loads the artifact's init payload (here: model json), `run` infers
//! on canonical-JSON input bytes. Every failure traps — an honest execution
//! failure. Zero imports by construction.

use std::sync::OnceLock;

use auto_model::Model;
...
/// Load the model (canonical model json). Once per instance, before `run`.
...
pub extern "C" fn init(ptr: u32, len: u32) {
    ...
    let text = std::str::from_utf8(bytes).expect("model bytes must be utf-8");
    let model = Model::from_json(text).expect("model json must parse");
    MODEL
        .set(model)
...
pub extern "C" fn run(ptr: u32, len: u32) -> u64 {
    let model = MODEL
        .get()
        .expect("run requires init to have loaded a model");
    let bytes = read_region(ptr, len);
    ...
    // pure tree walk over counted trigram features; no code execution
    let output = auto_model::infer(model, &input).expect("inference failed");
