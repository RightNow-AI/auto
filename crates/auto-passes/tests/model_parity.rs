//! Differential parity: the embedded model interpreter
//! ([`auto_passes::model_interpreter_wasm`]) against native inference
//! (`auto_model::infer`) — one implementation, two compilations, byte-equal
//! outputs. Also proves the physical purity claim (zero imports) and that
//! every inference failure path traps instead of answering.
//!
//! Host driving mirrors the frozen ABI (spec/artifact.md §4) plus the `init`
//! extension — for distilled artifacts the init payload is MODEL json:
//! alloc → write model.json → init → alloc → write canonical input → run →
//! unpack `(out_ptr << 32) | out_len` → bounds-check → read. Plain engine,
//! no fuel: inference is a cycle-bounded tree walk, so every test call
//! terminates.

use std::sync::OnceLock;

use auto_passes::auto_model::{self, Features, Model, Node, fnv1a_32};
...
fn shared_module() -> &'static (Engine, Module) {
    ...
    MODULE.get_or_init(|| {
        ...
        let module = Module::new(&engine, auto_passes::model_interpreter_wasm())
            .expect("embedded model interpreter compiles");
        (engine, module)
    })
}
...
impl Interp {
    ...
    /// Load model json (the distilled artifact's init payload). `Err` is a
    /// trap.
    fn init(&mut self, model_json: &str) -> wasmtime::Result<()> {
        let (ptr, len) = self.stage(model_json.as_bytes());
...
/// Drive a fresh instance through init(model json) + run; raw output bytes.
fn infer_wasm(model: &Model, input: &Value) -> Vec<u8> {
    let mut interp = Interp::new();
    interp
        .init(&model.to_json())
        .expect("canonical model json initializes");
...
fn assert_parity(model: &Model, input: &Value) {
    // not code evaluation: auto_model::infer is a counted-trigram featurize
    // plus a cycle-bounded tree walk — pure data, no I/O, no exec
    let native = auto_model::infer(model, input).expect("native inference succeeds");
    let native_bytes = serde_json::to_string(&native).expect("value serialization cannot fail");
    let wasm_bytes = infer_wasm(model, input);
    assert_eq!(
        wasm_bytes.as_slice(),
...
/// Hand-built model over object inputs (`input_field = "text"`): one split
/// on the trigram "foo" — absent (count <= 0.5) goes left to "miss",
/// present goes right to "hit".
fn fielded_model() -> Model {
    Model {
        features: Features {
...
/// Hand-built model over bare-string inputs (`input_field = None`), two
/// levels: the root splits on "the"; its left subtree splits on "caf".
fn bare_model() -> Model {
    Model {
        features: Features {
            kind: "char_trigram_fnv1a".into(),
        ...
        nodes: vec![
            Node::Split {
                feature: bucket("the", BUCKETS),
                threshold: 0.5,
                left: 1,
                right: 2,
            },
            Node::Split {
                feature: bucket("caf", BUCKETS),
                threshold: 0.5,
                left: 3,
                right: 4,
            },
...
/// Parity, model with `input_field`: three inputs (a hit, a miss, and a
/// unicode text whose lowercasing must agree across the two compilations).
#[test]
fn parity_fielded_model_three_inputs() {
    let model = fielded_model();
    for (input, expected) in [
        ...
        assert_eq!(
            auto_model::infer(&model, &input),
            Ok(expected),
            "native behavior pinned for {input}"
        );
        assert_parity(&model, &input);
    }
}

/// Parity, model without `input_field` (the input IS the text): three bare
/// strings — one per leaf, including the empty string (no trigrams at all)
/// and a unicode input (İ lowercases to two chars).
#[test]
fn parity_bare_model_three_inputs() {
...
fn run_before_init_traps() {
    ...
    assert!(
        ...
        "run without a loaded model must trap"
    );
}

#[test]
fn init_with_invalid_model_traps() {
    assert!(
        Interp::new().init("not json").is_err(),
        "malformed model bytes must trap"
    );
    // structurally invalid: this build reads model_version 0 exactly
    let future = fielded_model()
        .to_json()
        .replace("\"model_version\":0", "\"model_version\":1");
    assert!(
        Interp::new().init(&future).is_err(),
        "unsupported model_version must trap"
    );
}

#[test]
fn inference_error_traps() {
    // fielded model, input missing the field: native errors, wasm traps
    let model = fielded_model();
    assert!(
        auto_model::infer(&model, &json!({"other": "foo"})).is_err(),
        "native side errors on the same case"
    );
    let mut interp = Interp::new();
    interp.init(&model.to_json()).expect("init succeeds");
    assert!(
...
fn second_init_traps() {
    let model_json = fielded_model().to_json();
    let mut interp = Interp::new();
    interp.init(&model_json).expect("first init succeeds");
    assert!(
        interp.init(&model_json).is_err(),
